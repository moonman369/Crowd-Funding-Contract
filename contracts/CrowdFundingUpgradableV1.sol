...
/// @title CrowdFundingUpgradableV1
/// @author Ayan Maiti
...
/// @dev This is a simple upgradable contract. DFND token contract must be deployed and its address must be passed as a parameter while calling this contract's initializer

contract CrowdFundingUpgradableV1 {
    ...
    /// @notice Ensures initializer is called only once
    ...
    /// @dev Initializer function is called when an upgradableProxyContract is deployed
    /// @param _erc20Address: Address of DFND ERC20 token contract
    function setTokenAddress(address _erc20Address) public {

        // Throws if initializer is already called once
        require(
            !initializerCalled,
            "DeFund: Cannot call initializer more than once"
        );
        initializerCalled = true; // Sets initializerCalled to true
        dfnd = DeFundToken(payable(_erc20Address)); // Initializes DFND token instance
    }
