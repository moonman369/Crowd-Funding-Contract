...
describe("III. Withdrawing collected funds (in case campaign goal was met)", () => {
  ...
  it("3. Owner SHOULD NOT be able to withdraw funds if invalid campaign is passed", async () => {
  ...
  it("4. Owner SHOULD NOT be able to withdraw collected funds before deadline.", async () => {
    await crowdFunding.connect(donor1).donateToCampaign(campaignId, 50);
    await crowdFunding.connect(donor2).donateToCampaign(campaignId, 50);
  ...
  it("5. Only campaign owner SHOULD be able to withdraw collected funds.", async () => {
    await crowdFunding.connect(donor1).donateToCampaign(campaignId, 50);
    await crowdFunding.connect(donor2).donateToCampaign(campaignId, 50);
  ...
  it("6. Owner SHOULD NOT be able to withdraw funds if campaign goal was not reached before deadline", async () => {
    await crowdFunding.connect(donor1).donateToCampaign(campaignId, 50);
  ...
  it("7. Owner SHOULD NOT be able withdraw funds more than once", async () => {
    await crowdFunding.connect(donor1).donateToCampaign(campaignId, 50);
    await crowdFunding.connect(donor2).donateToCampaign(campaignId, 50);
...
describe("IV. Withdrawing donated funds (in case campaign goal was not met", () => {
  ...
  it("4. Donors SHOULD NOT be able to withdraw funds before deadline", async () => {
    await crowdFunding.connect(donor1).donateToCampaign(campaignId, 50);
    await expect(
      crowdFunding.connect(donor1).withdrawDonatedFunds(campaignId)
    ).to.eventually.be.rejectedWith(
      "DeFund: Cannot withdraw funds before deadline."
    );
  });

  it("5. Donors SHOULD NOT be able to withdraw funds from campaigns they have not donated to.", async () => {
    await crowdFunding.connect(donor1).donateToCampaign(campaignId, 50);

    await sleep(10);

    await expect(
      crowdFunding.connect(donor2).withdrawDonatedFunds(campaignId)
    ).to.eventually.be.rejectedWith(
      "DeFund: No funds were donated to this campaign by caller."
    );
  });

  it("6. Donors SHOULD NOT be able to withdraw funds from a campaign more than once", async () => {
    await crowdFunding.connect(donor1).donateToCampaign(campaignId, 50);

    await sleep(10);

    await crowdFunding.connect(donor1).withdrawDonatedFunds(campaignId);
  ...
  it("7. Donors SHOULD NOT be able to withdraw funds if campaign goal was reached", async () => {
    await dfnd.connect(donor1).approve(crowdFunding.address, 500);
    await crowdFunding.connect(donor1).donateToCampaign(campaignId, 500);
    await crowdFunding.connect(donor2).donateToCampaign(campaignId, 50);
    ...
    await expect(
      crowdFunding.connect(donor1).withdrawDonatedFunds(campaignId)
    ).to.eventually.be.rejectedWith("DeFund: Campaign goal was met.");
  });
});
